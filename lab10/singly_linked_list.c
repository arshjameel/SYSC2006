...
int length(intnode_t *head)
{
    ...
    return count; //counts num of nodes
}
...
{
    int nodes = 0;  
    _Bool check_target = 0; //tests if target exists as a node value
...
void extend(intnode_t *head, intnode_t *other)
{
    ...
    if (head == NULL) {head = other; return;}

    intnode_t *tail;
    for (tail = head; tail->next != NULL; tail = tail->next) {} //do nothing, just iterate to the end

    for (intnode_t *curr = other; curr != NULL; curr = curr->next) {
        tail->next = push(tail->next, curr->value);
        tail = tail->next;
    }
} 
